...
impl Channels {
    ...
    // JUBEOR: XI Dispatching packets internaly. An event will be
    fn dispatch_inner(
        &mut self,
        packet: Packet,
        s: &mut TrafSend,
        ...
        match packet {
            ...
            Packet::ChannelRequest(p) => {
                ...
                match self.get_mut(ChanNum(p.num)) {
                    Ok(ch) => {
                        ev = ch.dispatch_request(&p, s, is_client); // JUBEOR: V p contains the ChannelRequest
                    }
    ...
    // JUBEOR: VIII Incoming packet handling starts here? At this point the inner dispatch is called and the result is handled
...
impl Channel {
    ...
    // JUBEOR : IV dispatching request client or server
    fn dispatch_request(
        &mut self,
    ...
    // JUBEOR: III Returning ChannelReqType from server request!!
    fn dispatch_server_request(
        ...
        match &p.req {
            ...
            // JUBEOR: Suggest I. Whatever is happening here, I need to identify this as what it is
            #[cfg(feature = "sftp")]
            ChannelReqType::Subsystem(_) => {
                Ok(DispatchEvent::ServEvent(ServEventId::SessionSub))
            }
            _ => {
