...
impl<'g, 'a> Event<'g, 'a> {
    pub(crate) fn from_dispatch(
        ...
        match disp {
            ...
            DispatchEvent::ServEvent(x) => Ok(Self::Serv(x.event(runner)?)), // JUBEOR : XIII from_dispatch Chasing the source of the events.
...
pub enum ServEvent<'g, 'a> {
    ...
    #[cfg(feature = "sftp")] // JUBEOR: Adding SFTP to ServEvent
    SessionSftp(ChanRequest<'g, 'a>),
...
impl Debug for ServEvent<'_, '_> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let e = match self {
            ...
            Self::Defunct => "Defunct",
            #[cfg(feature = "sftp")] // JUBEOR: Adding SFTP to ServEvent
            Self::SessionSftp(_) => "SessionSFTP",
        };
        write!(f, "ServEvent({e})")
    }
}
...
impl ServEventId {
    pub fn event<'g, 'a>(
        // JUBEOR: XII Chasing the source of the events
        self,
        runner: &'g mut Runner<'a>,
        ...
        match self {
            ...
            #[cfg(feature = "sftp")] // JUBEOR: Adding SFTP to the event
            Self::SessionShell => {
                debug_assert!(matches!(p, Some(Packet::ChannelRequest(_))));
                Ok(ServEvent::SessionSftp(ChanRequest::new(runner)))
            }
...
impl<'g, 'a> ChanRequest<'g, 'a> {
    ...
    // JUBEOR: I Following the req channel
